...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://saas.networkspecialist.in:8007"],
...
def discover_service(service_name: str) -> str:
    ...
    return f"http://{chosen['Service']['Address']}:{chosen['Service']['Port']}"

@app.api_route("/{prefix}/{full_path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def gateway(prefix: str, full_path: str, request: Request):
    service_name = SERVICE_MAP.get(prefix)
    if not service_name:
        return Response(content=f"Unknown service prefix '{prefix}'", status_code=404)
