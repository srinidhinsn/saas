...
def create_inventory(
    ...
    return ResponseModel[Inventory](
        screen_id=context.screen_id, status="success", message="Inventory item created", data=model
    )
...
def update_inventory(
    ...
    record = (
        ...
        .filter(InventoryEntity.id == updates.id, InventoryEntity.client_id == client_id)
        .first()
    )
    if not record:
    ...
    return ResponseModel[Inventory](
        screen_id=context.screen_id, status="success", message="Inventory item updated", data=model
    )
...
def delete_inventory(
    ...
    record = (
        db.query(InventoryEntity)
        .filter(InventoryEntity.id == item.id, InventoryEntity.client_id == client_id)
        .first()
    )
    if not record:
        raise HTTPException(status_code=404, detail="Inventory item not found")
    ...
    model = InventoryEntity.copyToModel(record)
    return ResponseModel[Inventory](
        screen_id=context.screen_id, status="success", message="Inventory item deleted", data=model
    )
...
def delete_all_inventory(
    ...
    records = db.query(InventoryEntity).filter(InventoryEntity.client_id == client_id, InventoryEntity.realm == "restaurant").all()
...
def read_categories(
    ...
    records = (
        ...
        .filter(CategoryEntity.client_id == client_id)
        .order_by(CategoryEntity.slug)
        .all()
    )
    models = CategoryEntity.copyToModels(records)
    category_tree = service.build_category_tree(models, category_id)
    return ResponseModel(
        screen_id=context.screen_id, status="success", message="Fetched categories", data=category_tree
    )
...
def create_category(
    ...
    return ResponseModel[Category](
        screen_id=context.screen_id, status="success", message="Category created", data=model
    )
...
def update_category(
    ...
    record = (
        db.query(CategoryEntity)
        .filter(CategoryEntity.id == updates.id, CategoryEntity.client_id == client_id)
        .first()
    )
    if not record:
    ...
    db.query(CategoryEntity).filter(
        CategoryEntity.id == updates.id, CategoryEntity.client_id == client_id
    ).update(updates.dict(exclude_unset=True), synchronize_session=False)

    db.commit()
    db.refresh(record)
    model = CategoryEntity.copyToModel(record)
    return ResponseModel[Category](
        screen_id=context.screen_id, status="success", message="Category updated", data=model
    )
...
def delete_category(
    ...
    record = (
        db.query(CategoryEntity)
        .filter(CategoryEntity.id == category.id, CategoryEntity.client_id == client_id)
        .first()
    )
    if not record:
        raise HTTPException(status_code=404, detail="Category not found")
    ...
    model = CategoryEntity.copyToModel(record)
    return ResponseModel[Category](
        screen_id=context.screen_id, status="success", message="Category deleted", data=model
    )
