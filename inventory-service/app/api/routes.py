...
@router.get("/inventory/read", response_model=ResponseModel[List[Inventory]])
def read_inventory(client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[List[Inventory]](screenId=context.screenId, status="success", message="Fetched inventory list", data=models)

@router.post("/inventory/create", response_model=ResponseModel[Inventory])
def create_inventory(item: Inventory, client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[Inventory](screenId=context.screenId, status="success", message="Inventory item created", data=model)

@router.post("/inventory/update", response_model=ResponseModel[Inventory])
def update_inventory(client_id: str, updates: Inventory, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[Inventory](screenId=context.screenId, status="success", message="Inventory item updated", data=model)

@router.post("/inventory/delete", response_model=ResponseModel[Inventory])
def delete_inventory(client_id: str, item: Inventory, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    db.commit()
    model = InventoryEntity.copyToModel(record)
    return ResponseModel[Inventory](screenId=context.screenId, status="success", message="Inventory item deleted", data=model)

# -------------------- CATEGORY ROUTES --------------------

@router.get("/category/read", response_model=ResponseModel[List[Category]])
def read_categories(client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[List[Category]](screenId=context.screenId, status="success", message="Fetched categories", data=models)

@router.post("/category/create", response_model=ResponseModel[Category])
def create_category(category: Category, client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[Category](screenId=context.screenId, status="success", message="Category created", data=model)

@router.post("/category/update", response_model=ResponseModel[Category])
def update_category(client_id: str, updates: Category, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    return ResponseModel[Category](screenId=context.screenId, status="success", message="Category updated", data=model)

@router.post("/category/delete", response_model=ResponseModel[Category])
def delete_category(client_id: str, category: Category, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    db.commit()
    model = CategoryEntity.copyToModel(record)
    return ResponseModel[Category](screenId=context.screenId, status="success", message="Category deleted", data=model)
