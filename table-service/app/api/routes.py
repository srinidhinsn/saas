...
def read_tables(client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    table_entities = db.query(DiningTable).filter(
        DiningTable.client_id == client_id).all()
    tables = DiningTable.copyToModels(table_entities)
    response = ResponseModel[List[Table]](
        screen_id=context.screen_id, data=tables)
    return response
...
def create_table(table: Table, client_id: str, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    response = ResponseModel[Table](
        screen_id=context.screen_id, data=table_model)
    return response
...
def update_table(client_id: str, updates: Table, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    db_table = db.query(DiningTable).filter(
        DiningTable.id == updates.id, DiningTable.client_id == client_id).first()
    ...
    table_model = DiningTable.copyToModel(db_table)
    response = ResponseModel[Table](
        screen_id=context.screen_id, status="success", message="Table updated successfully", data=table_model)
    return response
...
def delete_table(client_id: str, table: Table, context: SaasContext = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    db_table = db.query(DiningTable).filter(
        DiningTable.id == table.id, DiningTable.client_id == client_id).first()
    ...
    table_model = DiningTable.copyToModel(db_table)
    response = ResponseModel[Table](
        screen_id=context.screen_id, status="success", message="Table deleted successfully", data=table_model)
    return response
