  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const safeCategories = (catRes.data?.data || []).filter(c => c && typeof c === "object");

        const fullTree = safeCategories.filter(
    ...
    <div className="bg-bg-primary p-0 h-[calc(100vh-4rem)] overflow-x-hidden overflow-y-auto">
      {currentView === 'floor' && (
        <TableReservation tables={tables} onSelectTable={handleTableSelect} onSelectTakeaway={handleTakeawaySelect}
          ...
          onMarkAsServed={handleMarkAsServed} clientId={clientId} token={token} />
      )}
