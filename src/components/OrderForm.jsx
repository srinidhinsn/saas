...
const OrderForm = ({ clientId, tableId, onOrderCreated }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8000/api/v1/${clientId}/menu/combos`)
      .then((res) => setCombos(res.data))
      .catch((err) => console.error("Failed to fetch combos", err));
  }, [clientId]);
  ...
  const handleCancel = (index) => {
    if (index === 0) {
      ...
      const updated = [...items];
      updated[0] = { item_id: "", item_type: "item", quantity: 1 };
      setItems(updated);
    } else {
      // Remove other rows
      const updated = [...items];
  ...
  return (
    <div className="order-form-wrapper">
      ...
      <h2 className="order-form-heading">📝 Place New Order</h2>

      {items.map((item, index) => (
        <div key={index} className="order-form-row">
          <select
            value={item.item_id}
            onChange={(e) => handleItemSelect(index, e.target.value)}
      ...
      <div className="order-form-actions">
        <p className="order-total">Total Price: ₹ {total.toFixed(2)}</p>
        <button onClick={addRow} className="btn add-btn">+ Add Item</button>
        <button onClick={handleSubmit} className="btn submit-btn">Submit Order</button>
      </div>
    </div>
  );
};
