...
const TakeOrder = ({ clientId, token, onOrderUpdate, realm }) => {
  ...
  const handleAddMainItemWithSelectedAddons = (selectedAddonIds) => {
    ...
    // ✅ FIX 1: Store main item in localStorage
    if (activeOrderId) {
      localStorage.setItem(
        `order_${activeOrderId}_new_item_${mainItemUniqueKey}`,
        JSON.stringify({
          item_id: selectedMainItem.id,
          unique_key: mainItemUniqueKey,
          added_at: timestamp,
          batch_timestamp: batchTimestamp,
          quantity: 1,
        })
    ...
    selectedLineItems.forEach(addon => {
      ...
      // ✅ FIX 1: Store addon in localStorage
      if (activeOrderId) {
        localStorage.setItem(
          `order_${activeOrderId}_new_item_${addonUniqueKey}`,
          JSON.stringify({
            item_id: addon.id,
            unique_key: addonUniqueKey,
            added_at: addonTimestamp,
            batch_timestamp: batchTimestamp,
            quantity: 1,
            parent_item_key: mainItemUniqueKey,
          })
  ...
  // ✅ FIX 2: Remove from localStorage when item is deleted
  const removeFromCart = (itemId, uniqueKey = null) => {
    setHasNewItems(true);

    if (uniqueKey) {
      // ✅ FIX 2: Remove from localStorage
      if (activeOrderId) {
        const storageKey = `order_${activeOrderId}_new_item_${uniqueKey}`;
  ...
  // ✅ FIX 1: Improved canPlaceOrder logic
  const canPlaceOrder =
    orderMode === 'takeaway'
      ? cart.length > 0
      : activeOrderId
        ? hasNewItems && cart.some(item => item.batch_timestamp === currentBatchTimestamp)
        : selectedTable && cart.length > 0;
  ...
  const CartItemWithAddons = ({ group, onUpdateQuantity, onRemove, isOldItem }) => {
    const { main, addons } = group;
