...
// ─────────────────────────────────────────────────────────────────────────────
// OldItemDeleteModal — asks transaction type, then "Remove 1" or "Remove All"
// ─────────────────────────────────────────────────────────────────────────────

const OldItemDeleteModal = ({ isOpen, onClose, item, onRemoveOne, onRemoveAll }) => {
  const [transactionType, setTransactionType] = useState('ITEM_CANCELLED');
  const [reason, setReason] = useState('');
  const [removeQty, setRemoveQty] = useState(1);

  useEffect(() => {
    if (isOpen) {
      setTransactionType('ITEM_CANCELLED');
      setReason('');
      setRemoveQty(1);
    }
  }, [isOpen]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="rounded-lg w-full max-w-sm bg-white shadow-xl">
        <div className="px-6 py-4 border-b flex justify-between items-center">
          <h2 className="text-lg font-bold text-red-600">Remove Item</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <X size={20} />
          </button>
        </div>
        <div className="px-6 py-5 space-y-4">
          ...
          {/* Qty to remove — only show picker if qty > 1 */}
          {maxQty > 1 && (
            <div>
          ...
          {/* Transaction type */}
          <div>
            <p className="text-xs font-semibold text-gray-600 mb-2">Reason type</p>
            <div className="flex gap-2">
              <button
                onClick={() => setTransactionType('ITEM_CANCELLED')}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition
                  ${transactionType === 'ITEM_CANCELLED'
              ...
              <button
                onClick={() => setTransactionType('WASTAGE')}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition
        ...
        <div className="px-6 py-4 flex gap-3 bg-gray-50 rounded-b-lg">
          ...
          <button
            onClick={() => {
              if (isRemoveAll) {
                onRemoveAll(transactionType, reason);
              } else {
                onRemoveOne(transactionType, reason, removeQty);
              }
            }}
            className={`flex-1 py-2.5 rounded-lg font-medium text-sm text-white transition
              ${isRemoveAll
...
// ─────────────────────────────────────────────────────────────────────────────
// CHANGE 2: OldItemRow — previously placed items with delete button
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div className="space-y-1">
      <div className="flex items-start gap-2 p-3 rounded-xl border bg-white shadow-sm">
        ...
        <div className="flex items-center gap-2 self-center">
          ...
          {/* CHANGE 2: delete button for old items */}
          <button
...
const TakeOrder = ({ clientId, token, onOrderUpdate, realm }) => {
  ...
  // CHANGE 2: state for old-item delete modal
  ...
  const handleViewOrder = async (table) => {
    ...
    try {
      ...
      const reconstructedCart = (activeOrder.items || []).map(item => {
        ...
        return {
          id: Number(item.item_id),
          // order_item_id is the DB primary key of the Db_OrderItem_Entity row —
          // needed for DELETE /order_item/delete endpoint
  ...
  // CHANGE 1: addToCart — merge into existing new item if same id, no active order sub-order context
  const addToCart = (item, parentItemKey = null) => {
    setHasNewItems(true);
    let batch = currentBatchTimestamp;
    if (!batch) {
    ...
    // Merge if it's a top-level item (not an addon) — works for both first orders AND sub-orders
    if (!parentItemKey) {
      ...
      if (existingIndex !== -1) {
        ...
        if (!isMobile) setShowCart(true);
        // Return existing key so callers can reference it (addons won't be re-added)
  ...
  /**
   * Reload the active order from the server and rebuild the cart in-place.
   * Called after any server-side item mutation so the cart always mirrors the DB.
   * If the server returns no items (order was auto-deleted), go back to floor.
   */
  const reloadCartFromServer = useCallback(async (orderId) => {
    try {
      ...
      if (!order || !order.items?.length) {
        // Order was auto-deleted by backend (last item removed)
        setCart([]);
        setActiveOrderId(null);
        setActiveDineinOrderId(null);
        setHasNewItems(false);
      ...
      // Preserve any unsaved new items the waiter has already added locally
      setCart(prev => {
  ...
  /**
   * Remove 1 quantity from a placed (old) item.
   * Strategy: delete the existing DB row, then re-create it with qty - 1.
   * This avoids needing a separate update-quantity endpoint and works with
   * the existing /order_item/delete + /dinein/create-sub-order API surface.
   */
 const handleOldItemRemoveOne = async (transactionType, reason, removeQty = 1) => {
  ...
  /**
   * Remove all quantity of a placed (old) item — single DELETE call.
   * Backend handles order auto-deletion and table status update if it was the last item.
   */
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Place order — CHANGE 3: auto-print KOT after successful order placement
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const handlePlaceOrder = async () => {
    ...
    // Capture items to print KOT for before cart is cleared
    ...
    try {
      ...
      // CHANGE 3: auto-print KOT after order is placed successfully
                      ...
                      {cart.length === 0 ? (
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-3">
                            ...
                            {/* CHANGE 3: Place Order button — KOT prints automatically after this */}
                            <button
                              onClick={handlePlaceOrder}
                              disabled={!canPlaceOrder || isPlacingOrder}
                              className={`py-2 rounded-lg text-sm font-semibold
                            ...
                            {/* CHANGE 3: Print KOT button removed — KOT is now auto-printed on Place Order */}
                          </div>
                        </>
                      )}
                    </div>
