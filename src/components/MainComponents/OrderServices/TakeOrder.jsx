...
const TakeOrder = ({ clientId, token, onOrderUpdate, realm }) => {
  ...
  useEffect(() => {
    ...
    console.log("MENU CONFIG LOADED (TakeOrder):", clientId, config);
    setMenuConfig(config);
  }, [clientId]);

  // ✅ Updated fetchData with categorical filtering
  ...
  // ✅ Updated getFilteredItems to use category IDs
  const getFilteredItems = () => {
    const q = (searchQuery || '').trim().toLowerCase();
    let items = menuItems;
    ...
    // Apply category filter using IDs
  ...
  // ✅ Group items by immediate child categories and maintain order (from MenuManagement)
              ...
              <div className="lg:h-[calc(98dvh-4rem)] lg:overflow-y-auto pr-1">
                <CategoryTree
                  ...
                  onSelectCategory={(categoryName) => {
                    ...
                    const cat = categoriesFlat.find(c => c.name === categoryName);
