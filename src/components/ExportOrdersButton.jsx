...
import React, { useEffect, useState } from "react";
import '../styles/ExportOrderButtons.css'
...
const ExportOrdersButton = ({ clientId }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert("There are no orders yet today.");
        return;
      }
  ...
  return (
    <div className="export-container">
      <h3 className="export-title">📤 Export Orders to Excel</h3>
      ...
      <div className="export-filters">
        <button onClick={() => applyDateFilter("today")}>Today</button>
        <button onClick={() => applyDateFilter("month")}>This Month</button>
        <button onClick={() => applyDateFilter("6months")}>Last 6 Months</button>
        <button onClick={() => applyDateFilter("1year")}>Last 1 Year</button>
      </div>
      ...
      <button onClick={handleDownload} className="export-download">
        ⬇ Download Excel
      </button>
    </div>
  );
