...
//     const [screenId, setScreenId] = useState(localStorage.getItem("screen_id") || "default_user"); // ✅ detect current navbar mode

//     useEffect(() => {
//         if (!token || !clientId || screenId !== "user_v3") return;
//         fetch(`${import.meta.env.VITE_API_USER_SERVICE_URL}/${clientId}/users/realms`, {
//             headers: { Authorization: `Bearer ${token}` },
//         })
//             .then((res) => res.json())
//             .then((data) => {
//                 setRealms(data.data || []);
//                 if (data.data?.length > 0) setRealm(data.data[0]);
//             })
//             .catch(() => console.error("Failed to load realms"));
//     }, [clientId, token, screenId]);
...
//                                     <button
//                                         className={`realm-button ${realm === "" ? "active" : ""}`}
//                                         onClick={() => setRealm("")}
//                                     >
//                                         All
//                                     </button>
...
//                                         <button
//                                             key={r}
//                                             className={`realm-button ${realm === r ? "active" : ""}`}
//                                             onClick={() => setRealm(r)}
//                                         >
//                                             {r}
//                                         </button>
...
//     const realmId = "realm"; // replace with actual category id if you have one

//     fetch(`${import.meta.env.VITE_API_USER_SERVICE_URL}/${clientId}/users/realms?realm=${realmId}`, {
//       headers: { Authorization: `Bearer ${token}` },
//     })
//       .then((res) => res.json())
//       .then((data) => {
//         setRealms(data.data || []);
//         if (data.data?.length > 0) setSelectedRealm(data.data[0]);
//       })
//       .catch(() => console.error("Failed to load realms"));
//   }, [clientId, token, screenId]);
...
//           {/* ✅ Realm section only visible for NavbarC */}
//           {screenId === "user_v3" && (
//             <div className="realm-list">
//               {/* Desktop buttons */}
//               <div className="realm-buttons">
