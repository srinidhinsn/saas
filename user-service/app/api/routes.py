...
#     user = db.query(User).filter(User.username == userReq.username, User.clientId == clientId).first()
...
async def login_user(client_id: str, userReq: LoginRequest, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.username == userReq.username, User.client_id == client_id).first()
