...
def update_order_status(
    ...
    order = db.query(Order).filter(Order.id == order_id,
                                   Order.client_id == client_id).first()
    if not order:
...
def get_kds_orders(client_id: UUID, db: Session = Depends(get_db)):
    ...
    for order in orders:
        ...
        table = db.query(DiningTable).filter(
            DiningTable.id == order.table_id).first()
        ...
        for i in order.items:
            if i.item_type == "item":
                item = db.query(MenuItem).filter(
                    MenuItem.id == i.item_id).first()
                name = item.name if item else "Unknown Item"
            else:
                item = db.query(MenuCombo).filter(
                    MenuCombo.id == i.item_id).first()
...
@router.get("/{client_id}/orders/count")
def get_order_count(client_id: str, db: Session = Depends(get_db)):
    count = db.query(Order).filter(Order.client_id == client_id).count()
    return {"count": count}
