...
def update_category(db: Session, client_id: UUID, category_id: UUID, updates: MenuCategoryUpdate):
    db_cat = db.query(MenuCategory).filter_by(id=category_id, client_id=client_id).first()
    if not db_cat:
        return None
...
def update_item(db: Session, client_id: UUID, item_id: UUID, updates: MenuItemUpdate):
    db_item = db.query(MenuItem).filter_by(id=item_id, client_id=client_id).first()
    if not db_item:
        return None
    for key, value in updates.dict(exclude_unset=True).items():
