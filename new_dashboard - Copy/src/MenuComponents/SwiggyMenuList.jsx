...
//                   <h4>{item.name}</h4>
//                   <p className="menu-card-price">₹{item.swiggyPrice}</p>
...
//                     <input
//                       type="number"
//                       value={editingItem.swiggyPrice}
//                       onChange={(e) => setEditingItem({ ...editingItem, swiggyPrice: e.target.value })}
...
function SwiggyMenuItemList({ clientId, category }) {
  ...
  useEffect(() => {
    if (showUnavailableModal && items.filter(i => !i.isAvailableSwiggy).length === 0) {
      setShowUnavailableModal(false);
    }
  }, [items, showUnavailableModal]);
  ...
  const handleEditSave = async () => {
    const {
      id, name, description, price, dietary, gst, swiggyPrice, zomatoPrice,
      isAvailableSwiggy, isAvailableZomato, category_id, imageFile
    } = editingItem;
  ...
  const toggleAvailability = async (item) => {
    try {
      const res = await axios.put(
        `http://localhost:8000/api/v1/${clientId}/menu/items/${item.id}`,
        { ...item, isAvailableSwiggy: !item.isAvailableSwiggy }
      );
  ...
  const filteredItems = (category && category.name !== "All"
    ? items.filter(i => i.category_id === category.id)
    : items).filter(i => i.isAvailableSwiggy === true);
      ...
      <div className="menu-item-table-container">
        <div className="menu-item-header">
          ...
          <div className="btns">
            ...
            {items.some(i => i.isAvailableSwiggy === false) && (
              <button className="btn-add" onClick={() => setShowUnavailableModal(true)}>
        ...
        <div className="menu-grid-container">
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item) => (
              <div key={item.id} className={`menu-grid-card ${recentlyEditedId === item.id ? "highlight" : ""}`}>
                <div className="menu-card-image">
                  {item.image_url ? (
                    <img src={`http://localhost:8000${item.image_url}`} alt={item.name} />
                  ) : (
        ...
        {showAddModal && (
          <div className="modal-overlay" onClick={(e) => {
            ...
            <div className="menu-modal-content">
              <h3>Add New Menu Item</h3>
              <AddMenuItemForm clientId={clientId} categoryId={addTargetCategoryId} onAdd={handleAddNewItem} />
              <button className="btn-cancel modal-close-btn" onClick={() => setShowAddModal(false)}>Close</button>
            </div>
          </div>
        )}
          ...
          <div className="modal-overlay" onClick={(e) => {
            ...
            <div className="menu-modal-content">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleEditSave(); }} className="add-menu-item-form">
                <div className="form-entry-wrapper">
                  <div className="form-row">
                    <input value={editingItem.itemCode || ""} onChange={(e) => setEditingItem({ ...editingItem, itemCode: e.target.value })} placeholder="Item Code" className="form-input short" required />
                    <input value={editingItem.name} onChange={(e) => setEditingItem({ ...editingItem, name: e.target.value })} placeholder="Item Name" className="form-input" required />
                    <input value={editingItem.description} onChange={(e) => setEditingItem({ ...editingItem, description: e.target.value })} placeholder="Description" className="form-input" />
                    <input type="file" accept="image/*" onChange={(e) => setEditingItem({ ...editingItem, imageFile: e.target.files[0] })} className="form-input" />
                    <select value={editingItem.dietary} onChange={(e) => setEditingItem({ ...editingItem, dietary: e.target.value })} className="form-input">
                      <option value="">Select Dietary</option>
                      <option value="VEG">VEG</option>
                      <option value="NON-VEG">NON-VEG</option>
                    </select>
                  ...
                  <div className="form-row">
                    <input type="number" value={editingItem.swiggyPrice} onChange={(e) => setEditingItem({ ...editingItem, swiggyPrice: e.target.value })} placeholder="Swiggy Price" className="form-input short" required />
                    <input type="number" value={editingItem.gst} onChange={(e) => setEditingItem({ ...editingItem, gst: e.target.value })} placeholder="GST (%)" className="form-input short" required />
                    <select value={editingItem.category_id} onChange={(e) => setEditingItem({ ...editingItem, category_id: e.target.value })} className="form-input">
                      {categories.map(cat => (
                        <option key={cat.id} value={cat.id}>{cat.name}</option>
                      ))}
                    </select>
        ...
        {showUnavailableModal && (
          <div className="modal-overlay" onClick={(e) => {
            if (e.target.classList.contains("modal-overlay")) setShowUnavailableModal(false);
          }}>
            <div className="menu-modal-content scrollable-modal">
              <h3>Unavailable Swiggy Items</h3>
              <div className="menu-grid-container">
                {items.filter(i => !i.isAvailableSwiggy).map(item => (
              ...
              <button className="btn-cancel modal-close-btn" onClick={() => setShowUnavailableModal(false)}>Close</button>
            </div>
          </div>
        )}
